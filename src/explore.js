...
async function getImage() {
  try {
    const response = await fetch(
      `https://pixabay.com/api/?key=${key}&orientation=vertical&safesearch=true&image_type=photo&page=2&per_page=100&category=music`
    );
...
async function getVideo() {
  try {
    const response = await fetch(
      `https://pixabay.com/api/videos/?key=${key}&orientation=vertical&safesearch=true&page=3&per_page=20&category=music`
    );
